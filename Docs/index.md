<br><br>
# **NIST Public Working Group Voter Records Interchange (VRI) Common Data Format Specification**

<br>

This document is a partial draft of a NIST specification on a common data format (CDF) for voter records interchange (VRI). It contains a brief introduction and a description of the UML model/XML schema.  Future sections will cover the use cases and contain additional detail.  
...
a fixed-font and angle brackets, e.g., `<ElectionReport>` or `<ReportingUnitType>`. Attributes, enumeration values, or other XML syntax are in a fixed-font, e.g., `label` or
